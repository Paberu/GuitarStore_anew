...
from shop.models import Section, Manufacturer, Product, Discount


def add_default_shop_data(request):
    sections = Section.objects.all().order_by('title')
    manufacturers = Manufacturer.objects.all().order_by('title')
    ...
    return {
        'sections': sections, 'manufacturers': manufacturers, 'search_form': search_form,
